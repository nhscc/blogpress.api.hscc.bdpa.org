    ...
    spec.map(([data, message], index) =>
      expect(
        rejectionFn(data).then(
          () => {
            return `error: resolved instead of rejected at index #${index}: ${JSON.stringify(
              data,
              undefined,
              2
...
describe('::createPage', () => {
  ...
  it('rejects if data is invalid or contains properties that violates limits', async () => {
    ...
    const newPages: [NewPage, string][] = [
      [undefined as unknown as NewPage, ErrorMessage.InvalidJSON()],
      ['string data' as NewPage, ErrorMessage.InvalidJSON()],
      [
        {} as NewPage,
    ...
    await expectRejectionsWithMatchingErrors(newPages, (data) =>
      Backend.createPage({
        data,
        __provenance: 'fake-owner',
        blogName: dummyAppData.users[2].blogName
...
describe('::updateBlog', () => {
  ...
  it('rejects if data is invalid or contains properties that violates limits', async () => {
    ...
    const patchBlogs: [PatchBlog, string][] = [
    ...
    await expectRejectionsWithMatchingErrors(patchBlogs, (data) => {
...
describe('::updatePage', () => {
  ...
  it('rejects if data is invalid or contains properties that violates limits', async () => {
    ...
    const patchPage: [PatchPage, string][] = [
    ...
    await expectRejectionsWithMatchingErrors(patchPage, (data) => {
